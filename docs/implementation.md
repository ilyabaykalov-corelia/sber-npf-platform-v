...
Process_pds_contract_approval создаёт Document и PdsContract пакетом с ref:createDocument. Пользовательские этапы: взятие оператором, обработка оператором, взятие согласующим и согласование. Процесс управляет статусами и ветвлениями; approvalStatus остаётся частью процессного контракта. Новая версия документа не создаёт новый процесс.
